...
import           Constraint.UnionFind           ( Point
                                                , equivalent
                                                , union'
                                                )
import           Constraint.Utils               ( throwTypeError
                                                , throwVariableScopeEscapeError
                                                )
...
adjustRank d k = do
  ...
  unless (st /= Generic) $ throwTypeError
    "adjustRank: equivalence class cannot have Generic status"
...
unifyDesc
  :: Queue s -> Descriptor s -> Descriptor s -> SolverM s (Descriptor s)
 ...
 where
  ...
  unifyStatus rank struc = do
    ...
    let oneRigidCase rigidRank = do
          when (rank < rigidRank)
            $ throwTypeError "can't lower rank of a rigid variable"
          unless (isLeaf struc) $ throwTypeError
    ...
    case (s1, r1, s2, r2) of
      ...
      (Rigid, _, Rigid, _) ->
        throwTypeError "can't unify two rigid type variables"
...
conjunctionList f l1 l2
  | length l1 /= length l2 = throwTypeError
