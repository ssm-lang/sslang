...
initSolverCtx prog = SolverCtx { currId     = 1
                               , currMark   = 1
                               , currNameId = 1
                               , solverProg = prog
...
initGenCtx Program { typeDefs = tdefs } =
  let cmap = foldl
        (\m (dcid, tcid, tvs, ts) -> M.insert dcid (dcid, tcid, tvs, ts) m)
        M.empty
        (concatMap tdef2dcon tdefs)
  in  SolverGen { dconMap = cmap }
 where
  tdef2dcon (tcid, tdef) =
    [ (dcid, tcid, targs tdef, getVariantArgTypes tv)
    | (dcid, tv) <- variants tdef
    ]
