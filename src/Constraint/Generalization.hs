...
initGen = do
  ...
  return Gen { genPool = ia, genYoung = y }
...
flexible
  :: Gen s -> Maybe (Structure (U.Variable s)) -> SolverM s (U.Variable s)
...
discoverYoungGeneration (Gen { genPool = pool, genYoung = young }) = do
  ...
  let
    f v = do
      ...
      unless (st /= U.Generic)
        $ throwTypeError "there shouldn't be generic type variables in pool"
      unless (U.baseRank <= r && r <= y)
        $  throwTypeError
        $  "illegal rank: "
        ++ show r
      IA.update arr r (v :)
      ...
      b2 <- do
        ...
        if r < y
          then register g v r >> return False
            ...
            unless (r == y)
              $ throwTypeError "generalize: rank must be the same as young"
...
trivial root =
  Scheme { schemeRoot = root, schemeGenerics = [], schemeQuantifiers = [] }
    ...
    mapping <- do
      zipWithM
        (\i v -> do
          ...
          st <- readSTRef $ U.descStatus d
          unless (st == U.Generic)
            $ throwTypeError "can only instantiate generic type variables"
