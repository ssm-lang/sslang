      ...
      wildCase = case samplePat pm of
        ...
        Just sample ->
                ...
                then
                  let
                    f ck = do
                      ck' <- askCInfo ck
                      let arity = cArity ck'
                      useful (PV.specializeWild arity pv)
          ...
          in  case sample of
                ...
                A.PatTup ps -> useful (PV.specializeWild (length ps) pv)
...
wellFormedWild pm | PM.nrow pm < 0 = throwMalformedError
                  | PM.nrow pm == 0 = return ()
