...
buildCtx tds = MatchCtx { typeMap = tmap, consMap = cmap }
    where
        ...
        tAcc td tmap' =
                let
                        typ   = A.typeName td
                        clist = map (\(A.VariantUnnamed cid _) -> cid)
                                    (A.typeVariants td)
                        cset = S.fromList clist
                in
                        M.insert typ
                                 (MatchT { tName = typ, tCSet = cset })
                                 tmap'
        cAcc td cmap' =
                let
                        typ   = A.typeName td
...
checkExpr (A.Match e arms) =
        ...
        in  checkExpr e >> checkExprs es >> checkPats ps
...
checkPats ps =
        ...
        in  void $ not <$> useful pm (justPV A.PatWildcard)
...
useful pm pv | getPMN pm < 0  = undefined
    ...
    where
        ...
        inductiveCase =
                let
                        wildcase = if getPMM pm == 0
                                then do
                                        dpm <- defaultPM pm
                                        useful dpm (tailPV pv)
                                else case cSet pm of
                                        Nothing -> do
                                                dpm <- defaultPM pm
                                                useful dpm (tailPV pv)
                                        Just cset ->
                        ...
                        case head (getPVL pv) of
                                ...
                                A.PatApp ((A.PatId i) : _) -> if isCons i
                                        then do
                                                ...
                                                throwError
                                                        $ PatternError
                                                                  "useful: can't have type variable start a PatApp"
                                A.PatApp _ ->
                                        throwError
                                                $ PatternError
                                                          "useful: malformed PatApp"
                        ...
                        A.PatApp (r : _) -> case r of
                                A.PatId i -> if isCons i
                                        then Just (True, i)
                                                ...
                                                error
                                                        "can't have variable be the first in PatApp"
                                _ ->
                                        error
                                                "can't have patterns other than PatId be the first in PatApp"
...
specializedPV arity pv =
        ...
        in
                case head (getPVL pv) of
                        A.PatWildcard ->
                                let
                                        sPats = replicate arity A.PatWildcard
                                                ++ tail (getPVL pv)
                        ...
                        A.PatAs _ _ ->
                                throwError
                                        $ PatternError
                                                  "specializedPV: can't specialize PatAs"
                        A.PatTup ps ->
                                return $ PV width $ ps ++ tail (getPVL pv)
                        A.PatApp ps ->
                                return $ PV width $ tail ps ++ tail (getPVL pv)
                        A.PatAnn _ _ ->
                                throwError
                                        $ PatternError
                                                  "specializedPV: can't specialize PatAnn"
        ...
        let     width = nElts + getPMN pm - 1
                ...
                specializedRowAcc pv pmAcc = case head (getPVL pv) of
                        ...
                        A.PatAs _ r -> do
                                ...
                                rows <- specializedPMTup
                                        nElts
                                        (row `consPV` emptyPM n)
                        ...
                        A.PatTup elts -> if length elts == nElts
                                then
                                        ...
                                        throwError
                                                $ PatternError
                                                          "specializedPMTup: tuple size is not the same"
        ...
        let     width = arity + getPMN pm - 1
                ...
                specializedRowAcc pv pmAcc = case head (getPVL pv) of
                        ...
                        A.PatApp [] ->
                                throwError
                                        $ PatternError
                                                  "specializedPMCons: PatApp is empty"
                        A.PatApp (p : rs) -> case p of
                                ...
                                _ ->
                                        throwError
                                                $ PatternError
                                                          "specializedPMCons: PatApp must start with a PatId"
                        A.PatLit _ -> return pmAcc
                        A.PatTup _ -> return pmAcc
        foldrM specializedRowAcc (emptyPM width) (getPML pm)
...
defaultPM pm =
        let
                ...
                defaultRowAcc pv pmAcc = case head (getPVL pv) of
                        A.PatWildcard ->
                                let ps = tail $ getPVL pv
                                in  return $ PV width ps `consPV` pmAcc
                        A.PatId c -> if isCons c
                                then return pmAcc
                                else
                                        let ps = tail $ getPVL pv
                        ...
                        A.PatAnn _ r -> do
                                ...
                                return $ rows `appendPM` pmAcc
        in
                foldrM defaultRowAcc (emptyPM width) (getPML pm)
