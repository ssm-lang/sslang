...
import qualified IR.Constraint.Constrain.Expression
                                               as Expr
...
constrainExternDecl (varId, canTyp) finalConstraint = do
  constrainDeclaration (Ident.fromId varId)
                       (Can.schemeOf canTyp)
                       finalConstraint
...
constrainTypeDefs tdefs finalConstraint = foldrM
...
constrainTypeDef tcon (I.TypeDef variants args) finalConstraint = foldrM
