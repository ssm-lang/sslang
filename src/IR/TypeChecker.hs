...
runInferFn (InferFn m) =
  evalStateT m TypeCtx { varMap = M.empty, dConMap = M.empty }
...
inferADT adts@((tconId, h) : tl) = do
 ...
 where
  ...
  insertADT (tcon, TypeDef { variants = vars }) = mapM_
    (insertDCon tcon . fst)
    vars
   where
    insertDCon :: I.TConId -> I.DConId -> InferFn ()
    insertDCon t d = modify
      $ \st -> st { dConMap = M.insert d (Classes.TCon t []) $ dConMap st }
  inferTypeDef :: TypeDef Ann.Type -> InferFn (TypeDef Classes.Type)
  inferTypeDef TypeDef { variants = vars, targs = ar } = case vars of
    []                   -> pure TypeDef { variants = [], targs = ar }
    ((dconId, h2) : tl2) -> do
      h2'                         <- inferTypeVariant h2
      TypeDef { variants = tl2' } <- inferTypeDef TypeDef { variants = tl2
                                                          , targs    = ar
                                                          }
      return TypeDef { variants = (dconId, h2') : tl2', targs = ar }
  inferTypeVariant
    :: TypeVariant Ann.Type -> InferFn (TypeVariant Classes.Type)
  inferTypeVariant (VariantUnnamed []) = do
    return (VariantUnnamed [])
  ...
  inferTypeVariant (VariantNamed []) = do
    return (VariantNamed [])
  ...
  case record of
    ...
    Just t'' ->
      throwError
        $  Compiler.TypeError
        $  fromString
        $  "Var expression has inconsistent type annotaation: "
        ++ show t'
        ++ ", expected "
        ++ show t''
...
inferPrim (I.Prim I.Drop [e, r] _) = do
  e' <- inferExpr e
  r' <- inferExpr r
  return $ I.Prim I.Drop [e', r'] $ extract e'
...
inferPrim e =
  throwError
    $  Compiler.TypeError
    $  fromString
    $  "Unable to type Prim expression: "
    ++ show e
...
anns2Class (Ann.Type []) = throwError $ Compiler.TypeError $ fromString
  "Cannot change empty Ann type to Classes type"
...
ann2Class (Ann.TVar _tid) =
  throwError $ Compiler.UnexpectedError $ fromString
    "Do not know how to translate type variable yet"
