...
repr :: Point s a -> InferM s (Point s a)
repr point@(Pt l) = do
  link <- readSTRef l
  case link of
    Info _           -> return point
...
-- descriptor.
descrRef :: Point s a -> InferM s (STRef s (Info a))
...
union :: Point s a -> Point s a -> InferM s ()
...
union' :: Point s a -> Point s a -> (a -> a -> InferM s a) -> InferM s ()
...
-- | equivalence class.
equivalent :: Point s a -> Point s a -> InferM s Bool
...
redundant :: Point s a -> InferM s Bool
