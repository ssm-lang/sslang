{- | Translate SSM top-level definition to C compilation unit.
...
genInit :: Type -> Maybe CIdent
genInit = fmap initialize_ . svBaseTypename

-- | Obtain the assign method for a given SSM scheduled variable 'Type'.
genAssign :: Type -> Maybe CIdent
genAssign = fmap assign_ . svBaseTypename

-- | Obtain the later method for a given SSM scheduled variable 'Type'.
genLater :: Type -> Maybe CIdent
genLater = fmap later_ . svBaseTypename
...
genStruct = do
 ...
 where
  ...
  structField (n, t) = [C.csdecl|$ty:t $id:n;|]
...
genEnter = do
  ...
  params  <- gets fnParams
  trigs   <- gets fnMaxWaits
  let act  = [C.cty|typename $id:act'|]
      act' = act_ actname -- hack to use this typename as expr in macros

      declParam (n, t) = [C.cparam|$ty:t $id:n|]
      initParam (n, _) = [[C.cstm|$id:acts->$id:n = $id:n;|]]
      initTrig (trigId, _) = [C.cstm|$id:acts->$id:trigId.act = $id:actg;|]

      enterParams =
        [ [C.cparam|$ty:act_t *$id:caller|]
          , [C.cparam|$ty:priority_t $id:priority|]
          , [C.cparam|$ty:depth_t $id:depth|]
          ]
          ++ map declParam (genParams params)
  return
    ( [C.cedecl|$ty:act_t *$id:(enter_ actname)($params:enterParams);|]
    , [C.cedecl|
...
genStep = do
  ...
  let act  = [C.cty|typename $id:actt|]
      actt = act_ actName -- hack to use this typename as expr in macros
  return
    ( [C.cedecl|void $id:(step_ actName)($ty:act_t *$id:actg);|]
    , [C.cedecl|
...
genExpr (L.Var n (L.TBuiltin (L.Arrow _ _))) =
  ...
  return ([C.cexp|$id:(enter_ n)|], [])
genExpr (L.Var n _) = do
  isLocal <- isLocalVar n
  if isLocal then return ([C.cexp|$id:acts->$id:n|], []) else todo
...
genExpr (L.Let [(Just n, d)] b _) = do
  ...
  let defInit = [C.citems| $id:acts->$id:n = $exp:defVal; |]
...
genExpr a@(L.App _ _ ty) = do
  ...
  tmpName                       <- nextTmp ty
  yield                         <- genYield
  let tmp = [C.cexp|$id:acts->$id:tmpName|]
      enterArgs =
        [[C.cexp|$id:actg|], [C.cexp|$id:actg->$id:priority|]]
          ++ argVals
          ++ [[C.cexp|&$exp:tmp|]]
      call = [C.citems|$id:activate($exp:fnEnter($args:enterArgs));|]
...
genPrim L.New [e, _] refType = do
  ...
  let alloc = [C.citems|
          ...
          $id:initialize($id:acts->$id:tmp);
          *$id:acts->$id:tmp.$id:value = $exp:val;
        |]
  return ([C.cexp|$id:acts->$id:tmp|], stms ++ alloc)
...
genPrim L.Deref [a] _ = do
  ...
  return ([C.cexp|$exp:val->$id:value|], stms)
genPrim L.Assign [lhs, rhs] _ = do
  ...
  -- TODO: handle event type
  let assignBlock = [C.citems|
          $items:lhsStms
          $items:rhsStms
          $id:assign($exp:lhsVal, $id:actg->$id:priority, $exp:rhsVal);
        |]
...
genPrim L.After [time, lhs, rhs] _ = do
  ...
  -- TODO: handle event type
  let laterBlock = [C.citems|
          $items:timeStms
          $items:lhsStms
          $items:rhsStms
          $id:later($exp:lhsVal, $exp:rhsVal, $id:now() + $exp:timeVal);
        |]
...
genPrim L.Fork procs _ = do
  ...
  let numChildren = length procs
      forkArgs    = genForkArgs
        numChildren
        ([C.cexp|$id:actg->$id:priority|], [C.cexp|$id:actg->$id:depth|])
      checkNewDepth = [C.citems|
      ...
      genActivate ((prioArg, depthArg), a@(L.App _ _ ty)) = do
        ...
        let tmp = [C.cexp|$id:acts->$id:tmpName|]
            enterArgs =
              [[C.cexp|$id:actg|], prioArg, depthArg]
                ++ argVals
                ++ [[C.cexp|&$exp:tmp|]]
        return
          ( tmp
          , concat evalStms
          , [C.citem|$id:activate($exp:fnEnter($args:enterArgs));|]
          )
...
genPrim L.Wait vars _ = do
  ...
  let trigs = zip [1 :: Int ..] varVals
      sens (i, var) =
        [C.citem|$id:sensitize($exp:var, &$id:acts->$id:(trig_ i));|]
      desens (i, _) = [C.citem|$id:desensitize(&$id:acts->$id:(trig_ i));|]
  return (unit, concat varStms ++ map sens trigs ++ yield ++ map desens trigs)
genPrim L.Loop [b] _ = do
  (_, bodyStms) <- genExpr b
  return (unit, [C.citems|for (;;) { $items:bodyStms }|])
genPrim L.Break  []  _ = return (undef, [C.citems|break;|])
genPrim L.Return [e] _ = do
  (val, stms) <- genExpr e
  -- Assign to return argument and jump to leave
  let retBlock = [C.citems|
...
genLiteral (L.LitIntegral i    ) _ = return ([C.cexp|$int:i|], [])
genLiteral (L.LitBool     True ) _ = return ([C.cexp|true|], [])
genLiteral (L.LitBool     False) _ = return ([C.cexp|false|], [])
...
genPrimOp L.PrimAdd [lhs, rhs] _ = do
  ...
  return ([C.cexp|$exp:lhsVal + $exp:rhsVal|], stms)
...
genForkArgs numChildren (currentPrio, currentDepth) =
  [ let p = [C.cexp|$exp:currentPrio + ($int:(i-1) * (1 << $exp:d))|]
        d = [C.cexp|$exp:currentDepth - $exp:(depthSub numChildren)|]
    in  (p, d)
  | i <- [1 .. numChildren]
  ]
...
depthSub :: Int -> C.Exp
depthSub numChildren = [C.cexp|$int:ds|]
