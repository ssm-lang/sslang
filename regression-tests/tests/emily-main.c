  ...
  case 0:
    ...
    case 1:
      if (ssm_to_sv(cont->cout)->last_updated == ssm_now()) {
        ...
        if (ssm_unmarshal(ssm_deref(cont->cout)) == 0x7) {
            printf("\033[2J"); // clear the screen on unused "BELL" character
        }
        else {
            putchar(ssm_unmarshal(ssm_deref(cont->cout)));
            //fflush(stdout);
        }
      }
    }
...
struct ssm_act *__enter_main(struct ssm_act *caller, ssm_priority_t priority,
                           ssm_depth_t depth, ssm_value_t *cout, ssm_value_t *num, ssm_value_t *__ret);
...
int main(void) {
 ...
 // ssm_time_t stop_at = 500; // SSM_MILLISECOND * 500;
  ssm_value_t cout = ssm_new_sv(ssm_marshal(0));
  ssm_value_t num = ssm_new_sv(ssm_marshal(0));
  ssm_value_t ret;

  {
    ...
    ssm_dup(cout);
    ssm_activate(__enter_main(&ssm_top_parent, new_priority, new_depth, &cout, &num, &ret));
    new_priority += pinc;
    ssm_dup(cout);
